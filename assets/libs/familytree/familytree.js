...
function d3_append_oneline_text(d3element, text, delimiter = "_", css_class = undefined, line_sep = 14,
    line_offset = undefined, x = 13, dominant_baseline = "central"){
    // adds a one-line text label to a d3 element
    if (!text) return;
    ...
    constructor(data, start_node_id = data.start) {
        ...
        if (data.links.length > 0) {
            ...
            this.nodes = this.dag.descendants().map(node => {
                if (node.id in data.unions) return new Union(node, this, data.unions[node.id])
                else if (node.id in data.persons) return new Person(node, this, data.persons[node.id]);
            });
        ...
        // if no edges but only nodes are defined: root = dag
            ...
            this.root = new Person(this.root, this, root_data);
            this.root.visible = true;
            this.number_nodes = 1;
...
class Union extends FTNode {
    ...
    constructor(dagNode, ft_datahandler, union) {
        // 改寫資料來源 在 init instance 傳進來 => data.unions[dagNode.id]
        super(dagNode.id, union);
        // link to new object
        dagNode.ftnode = this;
    ...
    add_parent(person_data) {
        ...
        const person = new Person(dagNode, this.ft_datahandler, person_data);
...
class Person extends FTNode {
    ...
    constructor(dagNode, ft_datahandler, person) {
        // 調整原先 person 資料傳進來的方法 data.nodes[dagNode.id]
        super(dagNode.id, person);
    ...
    // get_tooltip_content() {
    //     var server_ipOutStr = this.data.server_ip.length > 70 ? this.data.server_ip.substring(0, 70) + "..." : this.data.server_ip;
    //     return [
    //         `公司別：${this.data.company || ''}`,
    //         `伺服器IP：${server_ipOutStr || ''}`,
    //         `資料庫：${this.data.database || ''}`,
    //         `結構名稱：${this.data.schema || ''}`,
    //         `資料表名稱：${this.data.table_name || ''}`,
    //         `資料表說明：${this.data.table_description || ''}`,
    //         `資料庫描述：${this.data.database_description || ''}`
    //     ];
    // }
    ...
    add_own_union(union_data) {
        ...
        const union = new Union(dagNode, this.ft_datahandler, union_data);
...
class FTDrawer {
    ...
    static default_tooltip_func(node) {
        ...
        // var content = node.get_tooltip_content().join("<br/>")
        // return `<span style='font-size: 16px'><b>${content}</b></span><br>`
    };
    ...
    draw(source = this.ft_datahandler.root) {
        ...
        nodeEnter.each(function(node) {
            ...
            //     "node-label",
            // )
            d3_append_oneline_text(
                d3.select(this),
                this_object.node_label_func(node),
                FTDrawer.label_delimiter,
                node.get_label_style(),
            );
            // "node-label"
        });
